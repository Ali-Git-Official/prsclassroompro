...
export default function App() {
  ...
  const getFormattedPersianDate = () => {
    try {
      ...
      const finalWeekday = weekday.replace(/[،,]/g, '').trim();
      const finalDay = toPersianDigits(day.trim());
      const finalMonth = month.replace(/[،,]/g, '').trim();
      const finalYear = toPersianDigits(year.trim());

      return `${finalWeekday} ${finalDay} ${finalMonth} ${finalYear}`;
    } catch (e) {
      return "جمعه ۲۲ خرداد ۱۴۰۵";
    }
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-tr from-slate-50 via-indigo-50/40 to-teal-50/30 flex flex-col items-center justify-center p-6 text-slate-800 font-sans overflow-hidden select-none relative">
        ...
        <div className="space-y-7 text-center max-w-sm w-full z-10 animate-fade-in">
          ...
          <div className="relative mx-auto w-24 h-24 rounded-full bg-white border border-indigo-100 shadow-xl flex items-center justify-center text-indigo-600 scale-100 hover:scale-105 transition-transform duration-500">
          ...
          <div className="bg-white/80 backdrop-blur-md p-5 rounded-3xl border border-slate-200/60 shadow-lg space-y-3.5">
            ...
            <div className="flex justify-between items-center text-[10px] text-slate-500 font-sans tracking-wide px-1">
              ...
              <span className="text-indigo-600 font-black font-sans">{splashProgress}%</span>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-between select-none">
      ...
      <footer className="bg-slate-900 text-slate-400 text-center py-4 text-[10px] border-t border-slate-800 font-sans" id="prs-footer">
        <p className="font-semibold text-xs flex items-center justify-center gap-2 flex-row-reverse flex-wrap" dir="rtl">
          <span className="text-slate-500 font-sans">P.R.S Classroom Pro</span>
          <span className="text-slate-600 font-sans">•</span>
          <span className="text-slate-300 font-sans">{persianDate}</span>
        </p>
      </footer>
