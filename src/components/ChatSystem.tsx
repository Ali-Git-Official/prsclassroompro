...
export default function ChatSystem({
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl flex flex-col h-[290px] overflow-hidden shadow-xs" id="prs-chat-system">
      {/* Small Header */}
      <div className="bg-slate-50 border-b border-slate-200 px-4 py-3 flex justify-between items-center text-right font-sans">
        <span className="text-[9px] text-teal-650 font-bold bg-teal-50 px-2 py-0.5 rounded-full flex items-center gap-1 font-sans">
          ...
          messages.map((msg) => {
            ...
            return (
              ...
              >
                ...
                {/* Bubble details */}
                <div className={`max-w-[78%] text-right font-sans ${isMe ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className={`p-2.5 rounded-xl text-xs space-y-1 ${
                    ...
                    <p className="leading-relaxed whitespace-pre-wrap font-sans text-start" dir="auto">{msg.text}</p>
                  </div>
                </div>
              </div>
            );
