...
export default function FileManager({
  ...
  // Annotation states for PDF Overlay
          ...
          <div className="bg-white w-full max-w-2xl rounded-xl border border-slate-200 overflow-hidden shadow-xl flex flex-col text-right">
            ...
            <div className="px-4 py-3 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
              <button 
                ...
                className="text-[10px] font-bold text-slate-600 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 px-2.5 py-1 rounded-md transition-colors cursor-pointer"
              >
                بستن پخش‌کننده
              </button>
              <span className="text-xs font-bold text-slate-800 truncate max-w-[300px] font-sans">{activeMediaName}</span>
            </div>

            {/* Content view based on file type */}
            <div className="bg-slate-950 aspect-video flex items-center justify-center relative overflow-hidden select-none">
              {activeMediaType === 'video' ? (
                activeMediaUrl === 'demo' ? (
                  <div className="p-6 text-center text-slate-450 flex flex-col items-center font-sans">
                ...
                <div className="w-full p-8 bg-slate-900 text-white flex flex-col items-center justify-center font-sans space-y-4">
                  <div className="w-16 h-16 rounded-full bg-slate-800 border-2 border-amber-500 flex items-center justify-center text-amber-500 shadow-xl relative animate-pulse">
                    ...
                    <span className="absolute inset-0 rounded-full border border-amber-501 animate-ping opacity-25"></span>
                  </div>
                ...
                <div className="relative w-full overflow-hidden flex flex-col bg-white" style={{ height: '520px' }}>
                  ...
                  <div className="absolute top-3 left-3 right-3 z-30 bg-slate-950/95 text-white rounded-xl p-2 flex items-center justify-between text-xs border border-slate-700/80 shadow-md">
                    ...
                    >
                      {pdfAnnotateActive ? '✏️ بستن تخته رسم' : '✏️ نوشتن و علامت‌گذاری روی PDF'}
                    </button>
                    
                    {pdfAnnotateActive && (
                      <div className="flex items-center gap-3">
                        <div className="flex gap-1.5">
                          {['#ef4444', '#3b82f6', '#10b981', '#fbbf24', '#000000'].map(clr => (
                            <button
                              key={clr}
                              type="button"
                              onClick={() => setPdfDrawColor(clr)}
                              className="w-4 h-4 rounded-full border border-white/60 shadow-inner"
                              style={{ backgroundColor: clr, transform: pdfDrawColor === clr ? 'scale(1.2)' : 'none' }}
                            />
                          ))}
                        </div>
                        ...
                        <button
                          ...
                          className="px-2 py-0.5 bg-slate-800 text-[10px] text-rose-350 hover:text-rose-100 hover:bg-rose-955 rounded border border-rose-900/50"
                        >
                          پاکسازی نشانه‌ها
                        </button>
                      </div>
                  ...
                  <div className="absolute bottom-3 right-3 z-10 bg-indigo-900/80 text-white rounded-full px-3 py-1 text-[10px] font-sans pointer-events-none">
                    نمای زنده PDF با ابزار هایلایتر و مداد تدریس
                  </div>
                </div>
              ) : (
                <div className="text-slate-400 flex flex-col items-center">
                  <HelpCircle size={32} />
                  <p className="text-xs font-sans">فرمت لود شده ناشناخته است.</p>
                </div>
              )}
            </div>
          </div>
