...
export default function FileManager({
  ...
  // Fullscreen support state variables
  ...
  return (
    <div className={`bg-white border border-slate-200 rounded-xl p-5 shadow-xs text-right font-sans transition-all duration-300 ${
      isMainFullscreen ? 'fixed inset-0 z-[9999] overflow-y-auto rounded-none bg-white p-8' : ''
    }`} id="prs-file-manager">
      <div className="flex justify-between items-center border-b border-slate-100 pb-3.5 mb-4 flex-wrap gap-2 text-right">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-500 hover:text-slate-800 transition-colors flex items-center justify-center cursor-pointer active:scale-95"
            title={isMainFullscreen ? "خروج از تمام صفحه منبع" : "تمام صفحه کردن پوشه جزوات و منابع"}
            id="toggle-main-file-fullscreen"
          >
            {isMainFullscreen ? <Minimize2 size={13} /> : <Maximize2 size={13} />}
          </button>
        </div>
                ...
                >
                  ...
                  <div className="text-right flex items-center gap-2.5">
                    ...
                    <div className="p-2 bg-slate-50 rounded-lg border border-slate-200 text-slate-600 animate-fade-in font-sans">
                      ...
                      {file.type === 'video' && <Video size={15} className="text-teal-600" />}
      ...
      {activeMediaUrl && (
        <div className={`fixed inset-0 bg-slate-950/85 backdrop-blur-xs z-[9999] flex items-center justify-center ${isViewerFullscreen ? 'p-0' : 'p-4'}`}>
          <div className={`bg-white rounded-xl border border-slate-200 overflow-hidden shadow-xl flex flex-col text-right ${isViewerFullscreen ? 'w-screen h-screen max-w-none rounded-none' : 'w-full max-w-2xl'}`}>
            {/* Header */}
            <div className="px-4 py-3 bg-slate-50 border-b border-slate-200 flex justify-between items-center flex-wrap gap-2 text-right">
              <div className="flex items-center gap-1.5">
                ...
                <button 
                  ...
                  className="p-1 px-1.5 hover:bg-slate-200 rounded-md text-slate-500 hover:text-slate-800 transition-colors flex items-center justify-center cursor-pointer active:scale-95"
                  title={isViewerFullscreen ? "کوچک کردن" : "تمام صفحه کردن نمایش منبع"}
                  id="toggle-media-viewer-fullscreen"
                >
                  {isViewerFullscreen ? <Minimize2 size={13} /> : <Maximize2 size={13} />}
                </button>
              </div>
            ...
            <div className="bg-slate-950 flex flex-col items-center justify-center relative overflow-hidden select-none w-full" style={{ height: activeMediaType === 'audio' ? 'auto' : (isViewerFullscreen ? 'calc(100vh - 60px)' : '520px') }}>
              ...
              <div 
                className="w-full h-full relative flex items-center justify-center overflow-auto transition-transform duration-100 animate-fade-in"
              ...
              >
                {activeMediaType === 'video' ? (
                  activeMediaUrl === 'demo' ? (
                    ...
                    <video 
                      ...
                      controls 
                      autoPlay
                      className={`w-full h-full object-contain ${isViewerFullscreen ? 'max-h-[85vh]' : 'max-h-[440px]'}`}
                    />
                  )
                  ...
                  <iframe 
                    ...
                    style={{ height: isViewerFullscreen ? 'calc(100vh - 120px)' : '440px' }}
                ...
                {pdfAnnotateActive && activeMediaType !== 'audio' && (
                  <canvas
                    ...
                    width={isViewerFullscreen ? 1600 : 760} 
                    height={isViewerFullscreen ? 1000 : 520} 
