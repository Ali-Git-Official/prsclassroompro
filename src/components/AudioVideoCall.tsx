...
export default function AudioVideoCall({
  ...
  return (
    <div className={isExpanded 
      ? "fixed inset-0 z-50 bg-slate-950 text-white p-6 overflow-y-auto flex flex-col justify-between font-sans text-right border-4 border-slate-800"
        ...
        <div className="flex items-center gap-2">
          {inCall && (
            <button
              ...
              className="px-2.5 py-1 bg-indigo-600/20 hover:bg-indigo-600 text-slate-100 text-[10px] font-bold rounded border border-indigo-500/30 transition-all cursor-pointer mr-2 active:scale-95"
            >
              {isExpanded ? '🗗 کوچک کردن مکالمه' : '🗖 بزرگ کردن گفتگوی صوتی و تماس'}
            </button>
          )}
                    ...
                    {role === 'teacher' ? (
                      <div className="flex gap-1.5 items-center">
                        <button
                          ...
                          className={`p-1 rounded cursor-pointer transition-colors ${studentPart.micActive ? 'bg-indigo-650/40 text-teal-400 hover:text-rose-500' : 'bg-rose-650 text-white'}`}
                        ...
                        <button
                          type="button"
                          onClick={() => handleTeacherForceControl(studentPart.id, studentPart.micActive, !studentPart.videoActive)}
                          className={`p-1 rounded cursor-pointer transition-colors ${studentPart.videoActive ? 'bg-indigo-650/40 text-teal-400 hover:text-rose-500' : 'bg-rose-650 text-white'}`}
