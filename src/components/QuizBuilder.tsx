...
export default function QuizBuilder({
  ...
  // Handle countdown setup when a student answers
  React.useEffect(() => {
    if (activeAnsweringQuizId) {
      const q = quizzes.find(item => item.id === activeAnsweringQuizId);
      if (q) {
        setQuizTimeLeft(q.durationMinutes * 60);
      }
    } else {
      setQuizTimeLeft(0);
    }
  }, [activeAnsweringQuizId, quizzes]);

  // Handle countdown ticker interval
  React.useEffect(() => {
    if (quizTimeLeft <= 0 || !activeAnsweringQuizId) return;
  ...
  }, [quizTimeLeft, activeAnsweringQuizId, quizzes]);
                        ...
                        <div className="flex items-center gap-2">
                          {onDeleteQuiz && (
                            <button
                              ...
                              className="p-1 text-slate-400 hover:text-rose-600 transition-colors cursor-pointer mr-1"
                              title="حذف کامل این آزمون"
              ...
              return (
                <div className="bg-white p-5 rounded-xl border border-slate-200 space-y-4">
                  <div className="flex justify-between items-center border-b border-indigo-50 pb-3 flex-wrap gap-2">
                    <div className="flex items-center gap-1.5 text-rose-600 bg-rose-50 border border-rose-100 px-3 py-1 rounded-full text-[11px] font-bold animate-pulse font-sans">
                      <Clock size={12} className="text-rose-500 shrink-0" />
                      <span>
                        زمان باقی‌مانده: {Math.floor(quizTimeLeft / 60)} دقیقه و {quizTimeLeft % 60} ثانیه
                      </span>
                    </div>
