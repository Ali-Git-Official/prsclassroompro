  ...
  return (
    <div className="min-h-[550px] flex items-center justify-center p-4 select-none font-sans text-right" id="prs-welcome-screen">
      <div className="w-full max-w-xl bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden flex flex-col p-6 sm:p-10 space-y-8 animate-fade-in relative">
        ...
        {/* Connection status and live shareable URL panel */}
        {externalClassUrl && (
          <div className="bg-teal-50/60 border border-teal-200/70 rounded-2xl p-4 text-slate-800 font-sans text-xs space-y-2.5 shadow-xs transition-all animate-fade-in">
            <div className="flex justify-between items-center flex-row-reverse">
              <span className="text-[9px] text-teal-700 font-extrabold bg-teal-100/60 px-2.5 py-0.5 rounded-full flex items-center gap-1.5 font-sans">
                <span className="w-1.5 h-1.5 rounded-full bg-teal-500 animate-ping"></span>
                آماده اتصال همگانی آنلاین (تحت وب)
              </span>
              <h4 className="font-extrabold text-[11px] text-teal-850">آدرس اشتراک‌گذاری اینترنت کلاس</h4>
            </div>
            <p className="text-[10px] text-slate-500 leading-relaxed text-right">
              دانش‌آموزان از سراسر جهان می‌توانند با وارد کردن این لینک در مرورگر خود، به این مجموعه درس آنلاین ملحق شوند:
            </p>
            <div className="flex bg-white/95 border border-teal-200/80 rounded-xl p-1.5 items-center justify-between gap-2 shadow-inner">
              <button
                type="button"
                onClick={() => {
                  try {
                    navigator.clipboard.writeText(externalClassUrl);
                ...
                className="bg-teal-600 hover:bg-teal-700 text-white font-extrabold px-3 py-1.5 rounded-lg text-[9px] transition-all cursor-pointer shrink-0 active:scale-95 shadow-sm"
              >
                {copiedWelcome ? "کپی شد! ✓" : "کپی آدرس کلاس"}
              </button>
              <span className="font-mono text-[10px] text-slate-600 overflow-x-auto scrollbar-none select-all text-left block w-full px-1 py-0.5 ltr">
                {externalClassUrl}
              </span>
            </div>
          </div>
